...
_hanldeEntityKilledIndex = addMissionEventHandler ["EntityKilled", { params ["_victim", "", "_instigator"];
...
_handleEntityRespawnedIndex = addMissionEventHandler ["EntityRespawned", { params ["_newEntity", "_oldEntity"];
	if (_oldEntity getVariable ["ocap_id", -1] != -1) then {
...
_handleHandleDisconnectIndex = addMissionEventHandler ["HandleDisconnect", { params ["_entity", "", "_uid", "_name"];
...
_handlePlayerConnectedIndex = addMissionEventHandler ["PlayerConnected", { params ["", "_uid", "_name"];
